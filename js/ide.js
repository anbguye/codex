...
async function handleRunError(jqXHR) {
  ...
  // Get the error message and current code
  ...
  // Add error to chat as a system message
  THREAD.push({
    role: "system",
    content: `The following code in ${language} failed to compile:\n\`\`\`\n${currentCode}\n\`\`\`\n\nError:\n${errorMessage}\n\nPlease analyze the error and suggest a fix.`,
  });

  // Get AI suggestion
  const aiMessage = document.createElement("div");
  aiMessage.classList.add("ui", "message", "judge0-message", "loading");
  if (!theme.isLight()) {
  ...
  require(["vs/editor/editor.main"], function (ignorable) {
    ...
    layout.registerComponent("source", function (container, state) {
      ...
      sourceEditor.onDidChangeCursorSelection((e) => {
        handleCodeSelection(sourceEditor);
      });
    });
...
function handleCodeSelection(editor) {
  ...
  if (!selection.isEmpty()) {
    ...
    chatButton.innerHTML = '<i class="comment icon"></i>';

    // Position the button near the selection
    const selectionPos = editor.getScrolledVisiblePosition(
      selection.getEndPosition()
    );
    if (selectionPos) {
      const editorPos = editor.getDomNode().getBoundingClientRect();
      chatButton.style.top = editorPos.top + selectionPos.top + "px";
      chatButton.style.left = editorPos.left + selectionPos.left + 10 + "px";
      ...
      editor.getDomNode().parentElement.appendChild(chatButton);

      // Remove button when selection changes
      setTimeout(() => {
        const currentSelection = editor.getSelection();
        if (!currentSelection.equalsRange(selection)) {
          chatButton.remove();
        }
      }, 100);
    }
